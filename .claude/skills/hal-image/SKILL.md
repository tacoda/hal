...
- **Inbox** — the user dropped image files into `inbox/` on disk. This is the
  work queue: anything sitting in `inbox/` is unprocessed by definition.

Pick the mode from the request. With no pasted image and no clear instruction, run
`ls inbox/`, show the pending files, and ask which to process (default: all).

## Steps (per image)

1. Get the image: read the pasted attachment, or `Read` the file from `inbox/`.
...
5. **Delete the image** — it is not stored:
   - Inbox file: `rm inbox/<file>` (clears it from the queue).
...
(`ls inbox/`) — every processed image should be gone.
