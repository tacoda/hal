...
- **Orchestrator-workers** — a central LLM dynamically breaks work into subtasks,
  ...
  [Cursor's agent swarm](../harness-engineering/cursor-agent-swarm-browser.md).)
- **Evaluator-optimizer** — one LLM generates, another critiques against clear
  ...
  [the loop-engineering playbook](../harness-engineering/loop-engineering-playbook.md) and the
  independent-verifier rule in [loop engineering](../harness-engineering/loop-engineering.md).
...
[loop engineering](../harness-engineering/loop-engineering.md) wraps an outer goal-seeking loop around.
