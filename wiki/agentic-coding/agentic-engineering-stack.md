...
```mermaid
flowchart TD
  A["1 · Developer Interface"] --> B["2 · Agent Orchestration"]
  B --> C["3 · Context — source of truth"]
  C --> D["4 · Model & Reasoning"]
  D --> E["5 · Tooling & Integrations (MCP)"]
  E --> F["6 · Execution & Infrastructure"]
```

## Cross-links

An architecture view complementing the five-layer runtime harness in
[Agent Harness Engineering](../harness-engineering/agent-harness-engineering.md) (orchestration, context,
tools/MCP, verification, operations all appear here as layers). See also
[Layers of AI](../misc/layers-of-ai.md) for the technique stack underneath the model layer.
